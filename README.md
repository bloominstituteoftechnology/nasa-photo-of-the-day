...
- [3 ] Download project dependencies by running `npm install`
- [4 ] Start up the app using `npm start`
- [ 5] Create a new branch: `git checkout -b <firstName-lastName>`.
- [ 6] Implement the project on your newly created `<firstName-lastName>` branch, committing changes regularly.
...
- [ 2] Once you have a design plan in mind, break down the designs into individual components.
- [3 ] Plan which components will hold state, what data each needs from props (if any), and where you will be making your data fetch.
- [ 4] Now it's time to jump into the code!

**File structure**

- [ 5] Take a look at your planned components. Create the folders and files you need for each component.
- [] Leave most of them blank for now - you need to get your data from the API before you can really get these built.

**Fetching the Data**

- [ 6] In `App.js` (or where ever you wanted to fetch the data) add state for the data you'll be getting from NASA.
- [ ] Add an effect hook to handle the
https://api.nasa.gov/#apod) API call side effect.
- [ 1] Go to the [NASA APOD API docs]( and read through the docs to see how to make the API call.
- [ 2] You don't _need_ an API key. However you may need one if you exceed the API request limits.
- [ 3] Using the endpoint given, fetch the data using `axios`.
- [ 4] In your `.then()` make sure to `console.log` the response so you can look at the shape of the data. 😃
- [ 5] Before you add your data to state, make sure your effect hook has a dependency array (probably empty, since we don't want this effect synced up to any state/props), otherwise you will start an **infinite loop, and you will exceed the API rate limits of the DEMO_KEY and need to use a real API_KEY.**
